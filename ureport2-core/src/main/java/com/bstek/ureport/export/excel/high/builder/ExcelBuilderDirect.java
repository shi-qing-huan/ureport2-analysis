		...
		try{
			...
			for(Row r:rows){
	        	...
	        	for(int i=0;i<columnSize;i++){
	        		...
	        		Object obj=cellInfo.getFormatData();//格式化后的显示值
	        		if(obj!=null){
	        			// FIXME 20211005 解决金额格式化后在excel中默认以字符串格式显示，不能用sum求和的问题
		        		if(obj instanceof Number){
		        			//格式化后的值==原值，即未曾被格式化
		        			BigDecimal bigDecimal=Utils.toBigDecimal(obj);
		        			cell.setCellValue(bigDecimal.doubleValue());//可能会存在精度丢失问题
		        			cell.setCellType(CellType.NUMERIC);
		        		}else if(cellInfo.getData() instanceof Number){
		        			// 已被格式化成字符串数值了，原值是数值型
		        			BigDecimal bigDecimal=Utils.toBigDecimal(cellInfo.getData());
		        			CellStyle cellStyle=cell.getCellStyle();
		        			DataFormat format=wb.createDataFormat();
		        			//FIXME 20211129 解决excel导出报ERR_INVALID_RESPONSE问题
		        			String formatStyle = cellInfo.getCellStyle().getFormat();
		        			if(formatStyle != null){
